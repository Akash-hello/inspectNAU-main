...
public class DatePickerControl : MonoBehaviour {
   ...
   // public GameObject DayscrollCUBE;
	...
	void Start () {
		...
		//fecha = System.DateTime.Now; // being fed from the class public class CalendarDateSave, this was done to load the date selected by the user...
		actualizarFecha();
	}
	...
	void Update () {
		...
		if (dateText != null) {
			if (formato == nFormato.Default) {
				dateText.text = dateStringFormato = DateGlobal.ToString(CultureInfo.InvariantCulture);
			} else if(formato == nFormato.custom){
				if (formatoCustom == "") {
					dateText.text = dateStringFormato = DateGlobal.ToString(formatoCustom, CultureInfo.InvariantCulture);

				} else {
					dateText.text = dateStringFormato = DateGlobal.ToString (formatoCustom, CultureInfo.InvariantCulture);
				}
			} else{
				dateText.text = dateStringFormato = DateGlobal.ToString (formato.ToString().Replace('_',separator), CultureInfo.InvariantCulture);
			}
		}
	}
